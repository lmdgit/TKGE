...
class TKBCModel(nn.Module, ABC):
    ...
    def get_ranking(
            ...
            batch_size: int = 1000, chunk_size: int = -1, RC_type: int = 1
    ):
        """
        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    if RC_type ==1:
                        q, ro1 = self.get_queries(these_queries)
                        scores = q @ rhs                     
                        ro1=ro1.unsqueeze(dim=1)
                        ro1=ro1.expand(ro1.size()[0], rhs.size()[1], ro1.size()[2])                    
                        rhs1=rhs.t().unsqueeze(dim=0)
                        rhs1=rhs1.expand(ro1.size()[0], rhs1.size()[1],rhs1.size()[2])                    
                        c_s=torch.cat([ro1, rhs1],-1)                    
                        score2= self.classifier1(c_s).squeeze()                                       
                        scores= scores + score2
                    else:
                        q, ro1, ro2 = self.get_queries(these_queries)

                        scores = q @ rhs - abs(ro1.unsqueeze(dim=-1) * rhs.unsqueeze(dim=0) - ro2.unsqueeze(dim=-1) ).sum(dim=1)
...
class TNTComplEx_SED(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            no_time_emb=False, init_size: float = 1e-2
    ):
        super(TNTComplEx_SED, self).__init__()
        self.sizes = sizes
        self.rank = rank

        self.embeddings = nn.ModuleList([
            nn.Embedding(s, 2 * rank, sparse=True)
            for s in [sizes[0], sizes[1], sizes[3], sizes[1], sizes[1], sizes[1]]  
        ])
    ...
    def score(self, x):
        ...
        ro1 = self.embeddings[4](x[:, 1])
        ro2 = self.embeddings[5](x[:, 1])        
        ...
        ro1 = ro1[:, :self.rank], ro1[:, self.rank:]
        ro2 = ro2[:, :self.rank], ro2[:, self.rank:]        
        ...
        return torch.sum(
            ...
            1, keepdim=True
        ) - torch.sum(abs(ro1[0] * rhs[0] - ro2[0]) + abs(ro1[1] * rhs[1] - ro2[1]), 1, keepdim=True)
    ...
    def forward(self, x):
        ...
        regularizer = (
           ...
           torch.sqrt(ro1[0] ** 2 + ro1[1] ** 2),
           torch.sqrt(ro2[0] ** 2 + ro2[1] ** 2)
        )
        #print(right[0].shape)
        #print(ro1[0].shape)
        #print(ro2[0].shape)
        ...
        return ((
               ...
               abs(ro1[0].unsqueeze(dim=-1) * right[0].t().unsqueeze(dim=0) - ro2[0].unsqueeze(dim=-1)).sum(dim=1) - abs(ro1[1].unsqueeze(dim=-1) * right[1].t().unsqueeze(dim=0) - ro2[1].unsqueeze(dim=-1)).sum(dim=1)
            ), regularizer,
    ...
    def get_queries(self, queries: torch.Tensor):
        ...
        ro1 = self.embeddings[4](queries[:, 1])
        ro2 = self.embeddings[5](queries[:, 1]) 
...
class TNTComplEx_MLP(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            no_time_emb=False, init_size: float = 1e-2
    ):
        super(TNTComplEx_MLP, self).__init__()
        self.sizes = sizes
        self.rank = rank
        
                
        self.classifier1 = MLP( self.rank * 4, 1).cuda()


        self.embeddings = nn.ModuleList([
            nn.Embedding(s, 2 * rank, sparse=True)
            for s in [sizes[0], sizes[1], sizes[3], sizes[1], sizes[1]]  
        ])
        self.embeddings[0].weight.data *= init_size
        self.embeddings[1].weight.data *= init_size
        self.embeddings[2].weight.data *= init_size
    ...
    def score(self, x):
        ...
        c_s=torch.cat([ro1,rhs],-1)
        score2= self.classifier1(c_s)
    ...
    def forward(self, x):
        ...
        ro1=ro1.unsqueeze(dim=1)
        ro1=ro1.expand(ro1.size()[0], right.size()[0], ro1.size()[2])
                    
        right1=right.unsqueeze(dim=0)
        right1=right1.expand(ro1.size()[0], right.size()[0],right.size()[1])
                    
        c_s=torch.cat([ro1, right1],-1)
        score2= self.classifier1(c_s).squeeze()
        ...
        return ((
               (lhs[0] * full_rel[0] - lhs[1] * full_rel[1]) @ right[0].t() +
               (lhs[1] * full_rel[0] + lhs[0] * full_rel[1]) @ right[1].t() + score2
            ), regularizer,
               self.embeddings[2].weight[:-1] if self.no_time_emb else self.embeddings[2].weight
        )
