...
bool _find(INT h, INT t, INT r, INT temp) {
    INT lef = 0;
    INT rig = tripleTotal - 1;
    INT mid;
    while (lef + 1 < rig) {
        INT mid = (lef + rig) >> 1;
        if ((tripleList[mid]. h < h) || (tripleList[mid]. h == h && tripleList[mid]. r < r) || (tripleList[mid]. h == h && tripleList[mid]. r == r && tripleList[mid]. t < t) || (tripleList[mid]. h == h && tripleList[mid]. r == r && tripleList[mid]. t == t && tripleList[mid]. temp < temp)) lef = mid; else rig = mid;
    }
    if (tripleList[lef].h == h && tripleList[lef].r == r && tripleList[lef].t == t && tripleList[lef].temp == temp) return true;
    if (tripleList[rig].h == h && tripleList[rig].r == r && tripleList[rig].t == t && tripleList[rig].temp == temp) return true;
    return false;
}

INT corrupt(INT h, INT r, INT temp){
	INT ll = tail_lef[r];
	INT rr = tail_rig[r];
	INT loop = 0;
	INT t;
