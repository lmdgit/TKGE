...
extern "C"
void getHeadBatch(INT *ph, INT *pt, INT *pr, INT *ptemp) {
    for (INT i = 0; i < entityTotal; i++) {
        ph[i] = i;
        pt[i] = testList[lastHead].t;
        pr[i] = testList[lastHead].r;
        ptemp[i] = testList[lastHead].temp;
    }
    lastHead++;
}

extern "C"
void getTailBatch(INT *ph, INT *pt, INT *pr, INT *ptemp) {
    for (INT i = 0; i < entityTotal; i++) {
        ph[i] = testList[lastTail].h;
        pt[i] = i;
        pr[i] = testList[lastTail].r;
        ptemp[i] = testList[lastTail].temp;
    }
    lastTail++;
}

extern "C"
void getRelBatch(INT *ph, INT *pt, INT *pr, INT *ptemp) {
    for (INT i = 0; i < relationTotal; i++) {
        ph[i] = testList[lastRel].h;
        pt[i] = testList[lastRel].t;
        pr[i] = i;
        ptemp[i] = testList[lastRel].temp;
    }
}
...
void testHead(REAL *con, INT lastHead, bool type_constrain = false) {
    ...
    INT temp = testList[lastHead].temp;
    INT lef, rig;
    if (type_constrain) {
        lef = head_lef[r];
        rig = head_rig[r];
...
void testTail(REAL *con, INT lastTail, bool type_constrain = false) {
    ...
    INT temp = testList[lastTail].temp;
    INT lef, rig;
    if (type_constrain) {
        lef = tail_lef[r];
        rig = tail_rig[r];
...
void testRel(REAL *con) {
    ...
    INT temp = testList[lastRel].temp;
