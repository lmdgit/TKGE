...
void* getBatch(void* con) {
	...
	INT *batch_temp = para -> batch_temp;
	...
	if (val_loss == false) {
		for (INT batch = lef; batch < rig; batch++) {
			...
			batch_temp[batch] = trainList[i].temp;
			batch_y[batch] = 1;
			INT last = batchSize;
			for (INT times = 0; times < negRate; times ++) {
				if (mode == 0){
					...
					if (randd(id) % 1000 < prob) {
						...
						batch_temp[batch + last] = trainList[i].temp;
					} else {
	...
	else
	{
		for (INT batch = lef; batch < rig; batch++)
		{
			...
			batch_temp[batch] = validList[batch].temp;
			batch_y[batch] = 1;
		}
	}
	pthread_exit(NULL);
...
void sampling(
	...
	for (INT threads = 0; threads < workThreads; threads++) {
		...
		para[threads].batch_temp = batch_temp;
