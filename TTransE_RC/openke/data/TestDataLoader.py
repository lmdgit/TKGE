...
class TestDataLoader(object):
	...
	def read(self):
		...
		self.tempTotal = self.lib.getTempTotal()
		...
		self.test_temp = np.zeros(self.entTotal, dtype=np.int64)
		self.test_h_addr = self.test_h.__array_interface__["data"][0]
		self.test_t_addr = self.test_t.__array_interface__["data"][0]
		self.test_r_addr = self.test_r.__array_interface__["data"][0]
		self.test_temp_addr = self.test_temp.__array_interface__["data"][0]
	...
	def sampling_lp(self):
		res = []
		self.lib.getHeadBatch(self.test_h_addr, self.test_t_addr, self.test_r_addr, self.test_temp_addr)
		res.append({
			"batch_h": self.test_h.copy(), 
			"batch_t": self.test_t.copy(), 
			"batch_r": self.test_r.copy(),
			"batch_temp": self.test_temp.copy(),
			"mode": "head_batch"
		})
		self.lib.getTailBatch(self.test_h_addr, self.test_t_addr, self.test_r_addr, self.test_temp_addr)
		res.append({
			"batch_h": self.test_h,
			"batch_t": self.test_t,
			"batch_r": self.test_r,
			"batch_temp": self.test_temp,
			"mode": "tail_batch"
		})
		return res
