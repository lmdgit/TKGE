    ...
    def convertTimes(self):      
        ...
        for split in ["train", "valid", "test"]:
            for i, fact in enumerate(self.data[split]):
                ...
                date = list(map(int, fact_date.split("-")))
                #ipdb.set_trace()
                token=list(map(int, str(date[0])))
                token.append(int(date[1]+9))
                day=list(map(int, str(date[2])))
                if len(day)<2:
                    token.append(int(22))
                    token.append(int(day[0]+22))
                else:
                    token.append(int(day[0]+22))
                    token.append(int(day[1]+22))
                    
                self.data[split][i] += token
